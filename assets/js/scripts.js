...
var NioApp = (function (jQ, win, doc){
    "use strict";
    var NioApp = {AppInfo: {name: "", package: "1.9.2", version: "1.0.4", author: ""} },
...
NioApp = function (NioApp, $, window, document) {
	...
	NioApp.Util.headerSticky = function () {
        ...
        stickyInit.hasShrink = function () {
            if($is_sticky.hasClass(_is_shrink)) {
                _headerHT = ($header.height() + 16 - 12); // navigation bar height
            }
        };
        
        stickyInit.hasFixed(); stickyInit.hasShrink();
        $win.on('resize', function() {
            _headerHT = ($is_sticky.hasClass(_is_shrink)) ? ($header.height() + 16 - 12) : ($header.outerHeight() - 12); // navigation bar height
        });
    ...
    NioApp.scrollAct = function() {
        ...
        $body.scrollspy({ 
            ...
            offset: (_headerHT + 12), // navigation bar height
        });
    };
